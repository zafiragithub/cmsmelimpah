// engine.js - VERSI STABIL (Apps Script Backend)

// Langsung menggunakan SCRIPT_URL dari config.js agar tidak terblokir
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        // 1. Ambil Pengaturan Global dari Apps Script
        ...
        if (currentPath === '' || currentPath === '/' || currentPath === '/index.html') {
            if (!homePageSlug) {
                ...
                pageBody.innerHTML = `<h2 class="text-2xl font-black text-slate-800">Selamat Datang di Web CMS! 🎉</h2><p class="text-slate-500 mt-2 font-medium">Sistem berjalan dengan baik. Silakan set Beranda Utama Anda dari Panel Admin.</p>`;
                return;
            }
        ...
        // Skenario B: Halaman Daftar Blog (/blog)
            ...
            // Tarik data artikel dari Apps Script
            const res = await fetch(API_URL, { 
                method: 'POST', 
                body: JSON.stringify({ action: 'get_public_posts' }) 
            });
            const data = await res.json();
            spinner.classList.add('hidden'); // Matikan loading
        ...
        // Skenario C: Baca Satu Artikel Penuh (/blog/slug-artikel)
            ...
            const res = await fetch(API_URL, { 
                method: 'POST', 
                body: JSON.stringify({ action: 'get_public_post_detail', slug: slug }) 
            });
            const data = await res.json();
            spinner.classList.add('hidden');
        ...
        // Skenario D: Halaman Statis Biasa (/slug-halaman)
        else {
        ...
        console.error("Koneksi ke Apps Script gagal:", error);
        spinner.classList.add('hidden');
        showError();
    }

    // Fungsi Pembantu: Render Halaman Statis Biasa
