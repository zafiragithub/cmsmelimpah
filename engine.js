// engine.js - VERSI PORTAL BERITA SEBAGAI HOMEPAGE (v11)
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // LOGIKA BARU: Tentukan apakah ini tampilan Portal Berita
        let isHome = currentPath === '' || currentPath === '/' || currentPath === '/index.html';
        let isBlogList = currentPath === '/blog' || isHome; // <--- INI KUNCINYA: Jika Home, anggap BlogList
        let isPostDetail = currentPath.startsWith('/blog/');
        
        let fetchPromises = [fetchSheet('Settings')];
        
        // Jika sedang buka portal berita atau detail artikel, tarik data 'Posts'
        if (isBlogList || isPostDetail) {
            fetchPromises.push(fetchSheet('Posts'));
        } else {
            fetchPromises.push(fetchSheet('Pages'));
        }
            ...
            <div class="bg-slate-900 text-slate-300 text-xs py-2 hidden md:block">
                <div class="max-w-7xl mx-auto px-4 flex justify-between items-center">
                    ...
                    <div class="flex items-center gap-4">
                        <span class="font-bold text-rose-500 flex items-center gap-1"><i data-lucide="trending-up" class="w-3 h-3"></i> TERBARU:</span>
                        <a href="/admin.html" class="hover:text-white transition-colors">Login Redaksi</a>
                    </div>
                </div>
            </div>
            ...
            <nav class="bg-white border-y border-slate-200 sticky top-0 z-50 shadow-sm">
                <div class="max-w-7xl mx-auto px-4">
                    <ul class="flex items-center gap-6 overflow-x-auto whitespace-nowrap text-[13px] font-bold uppercase tracking-wider text-slate-600 py-4 custom-scroll hide-scrollbar">
                        <li><a href="/" class="text-rose-600 border-b-2 border-rose-600 pb-4">Terkini</a></li>
        ...
        const htmlFooter = `
            <footer class="bg-slate-900 text-slate-300 py-12 border-t-4 border-rose-600 mt-10">
                <div class="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div><h4 class="text-white font-bold uppercase text-sm mb-4">Akses Cepat</h4><ul class="space-y-2 text-sm text-slate-400"><li><a href="/admin.html">Panel Admin</a></li><li><a href="/">Beranda</a></li></ul></div>
                    <div><h4 class="text-white font-bold uppercase text-sm mb-4">Ikuti Kami</h4><div class="flex gap-4"><a href="#" class="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center hover:bg-rose-600"><i data-lucide="instagram" class="w-4 h-4"></i></a><a href="#" class="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center hover:bg-rose-600"><i data-lucide="twitter" class="w-4 h-4"></i></a></div></div>
                </div>
                <div class="max-w-7xl mx-auto px-4 mt-12 pt-8 border-t border-slate-800 text-center text-xs text-slate-500">&copy; ${new Date().getFullYear()} ${siteName}.</div>
            </footer>
        `;
        ...
        if (isBlogList) {
            ...
            if (posts.length === 0) {
                html += `<div class="text-center py-20">Belum ada berita.</div>`;
            } else {
                html += `
            ...
            if (post) {
                ...
                let popular = contentData.slice(0, 5);
                let html = htmlTop + `
            ...
            if (page) { showCanvas(page[3]); } else { showError(); }
        }
